#使用 Hierarchical clustering (AgglomerativeClustering) 方法

import pandas as pd # 使用 pandas 讀檔和處理屬性下的值
import numpy as np #使用 numpy 計算陣列
from sklearn.cluster import AgglomerativeClustering #使用 AgglomerativeClustering 做分群
...
#使用KMeans分群
#並將分群結果放進clusters變數中
clusters = AgglomerativeClustering(n_clusters = 5).fit(matrix)
...
#將分群結果加總 (0 到 5)
for i in range(0, 5):
    x = np.sum(clusters.labels_ == i) # x 轉換成 i 群總和
